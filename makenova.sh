...
for host_info in "${hosts[@]}"; do
	...
	sed -i "s/HOSTIP/$ip/" "./$hostname/nova.conf"
	sed -i "s/LBHOST/$lbhost/" "./$hostname/nova.conf"
	sed -i "s/NOVAPASSWORD/$password/" "./$hostname/nova.conf"
done

password=$(grep 'PLACEMENT_PASSWORD' "$password_file" | cut -d'=' -f2)
for host_info in "${hosts[@]}"; do
	hostname="${host_info%%=*}"
	sed -i "s/PLACEMENTPASSWORD/$password/" "./$hostname/nova.conf"
done

password=$(grep 'NEUTRON_PASSWORD' "$password_file" | cut -d'=' -f2)
for host_info in "${hosts[@]}"; do
        hostname="${host_info%%=*}"
        sed -i "s/NEUTRONPASSWORD/$password/" "./$hostname/nova.conf"
done
...
for host_info in "${hosts[@]}"; do
	...
	sed -i "s@MEMCACHEDSTRING@$string@" "./$hostname/nova.conf"
done
...
for host_info in "${hosts[@]}"; do
        ...
        hostname="${host_info%%=*}"
        string=$(IFS=,; echo "${modified_hosts[*]}")
        sed -i "s#TRANSPORTSTRING#$string#" "./$hostname/nova.conf"
done
