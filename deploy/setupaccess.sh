...
for host_info in "${hosts[@]}"; do
	...
	if ssh -i "$PRIVKEY" -o "StrictHostKeyChecking no" "$ip" "echo '${loadbalancers#*=} ${loadbalancers%%=*}' | grep -q '${loadbalancers#*=} ${loadbalancers%%=*}' /etc/cloud/templates/hosts.debian.tmpl"; then
                echo "Loadbalancer already in hosts"
        else
                ssh -i $PRIVKEY -o "StrictHostKeyChecking no" $ip "sudo bash -c 'echo "${loadbalancers#*=} ${loadbalancers%%=*}" >> /etc/cloud/templates/hosts.debian.tmpl'"
        fi
	...
	for peer_ip in "${hosts[@]}"; do
                pip="${peer_ip#*=}"
                phostname="${peer_ip%%=*}"
                if ssh -i "$PRIVKEY" -o "StrictHostKeyChecking no" "$ip" "echo '$pip $phostname' | grep -q '$pip $phostname' /etc/cloud/templates/hosts.debian.tmpl"; then
                        echo "$phostname already in hosts"
                else
                        ssh -i $PRIVKEY -o "StrictHostKeyChecking no" $ip "sudo bash -c 'echo "$pip $phostname" >> /etc/cloud/templates/hosts.debian.tmpl'"
                fi
        done
done
